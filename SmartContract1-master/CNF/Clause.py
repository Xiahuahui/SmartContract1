...
class Clause:
    ...
    def assignLiteral(self, conLiteralID, status):
        literalSet =copy.deepcopy(self._literals)
        indexIds = {}
        for index in range(len(literalSet)):
            lit = literalSet[index]
        ...
        if len(indexIds) > 0:
            literalSet = []
            for index in range(len(self._literals)):
                if str(index) not in indexIds:
                    literalSet.append(self._literals[index])
            self._literals = copy.deepcopy(literalSet)
