...
class CNF:
    def __init__(self):
        ...
        # 即在字母表中 同一动作只有一个key,但同一条款会有多个key
        self._SATCore = []
        # 形如 [[1, -5, 4], [-1, 5, 3, 4]] 的 list，可以直接由pycosat求解
        # 在构造 cnf 以及每次更新 cnf 对象之后，均须更新 _SATCore
        self._isZero = False # 仅当clauseSet为空时生效 当为False为永真式
    ...
    # 更新前提  并对literal Id进行重新编号  TODO若也有
    # Input [[Term1：2], [Term2：3]] 字典
    # Output 无返回值
    # TODO 当是永真永假式,还存在问题 TODO若也有动作已经确定会发生错
    def updateCnf(self,combineChange):
        ...
        newClauseSet = []
        for cls in self._clauseSet:
            # print("cls 处理前",cls.toString())
            for conLitID in combineChange:          #可以减少次数
                if conLitID.startswith('Term'):
                    if combineChange[conLitID] >= 3:
                        # print("处理函数的两个参数:  ", "conLitID: ",conLitID,"combineChange: ",combineChange[conLitID])
                        cls.assignLiteral(conLitID, combineChange[conLitID])
                else:
                    if combineChange[conLitID] == 0 or combineChange[conLitID] == 1:
                        cls.assignLiteral(conLitID, combineChange[conLitID])
            # print("cls 处理后",cls.toString())
            if not cls.isEmpty():
    ...
    def _newLiteral(self, conLitID, val):
        ...
        if key in self._alphabet:
            ...
            if not conLitID.startswith('Term'):
                if val == 1:                       #当前judge不取非时:
                    oppokey = conLitID + '-0'
                else:
                    oppokey = conLitID + '-1'     #当前judge取非时
                    ...
                    if len(unit) == 0:
                        ...
                        if unit.startswith('Term'):
                            ...
                            if left not in mapping :
                                ...
                                mapping[left] = mapping[left] + 1
                            key = left + "#" +str(mapping[left])
                            ...
                            if act not in mapping :
                                ...
                                mapping[act] = mapping[act] + 1
                            key = act + "#"+str(mapping[act])
                        group[key] = val
            rlt.append(group)
    ...
    def _transKeyToConAssignment(self, key):
        ...
        for ch in re.split('[*]', key):
            ...
            for litKey in self._alphabet:
                ...
                if lit.isCMT():
                    if ('.' + dicStatus[lit.getStatus()]) not in rlt:
                        rlt[ '.' + dicStatus[lit.getStatus()]]

        # print('result for trans key(', key , '):', rlt)
        return list(rlt.keys())
...
if __name__ == '__main__':
    cnf = CNF()
    premise = 'judge(C,C2.vio)&&judge(B,C2.vio)'
    ...
    combinedChanges = {'judge(C,C2.vio)':1} #Input [[Term1：2], [Term2：3]] 字典
