...
func (c *ContractChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
  ...
  if function == "[A]预存书款100元" {
    return FsmEvent(stub, args, "[A]预存书款100元")
  } else if function == "[A]预存书款100元超时" {
    return FsmEvent(stub, args, "[A]预存书款100元超时")
  } else if function == "" {
    ...
    return FsmEvent(stub, args, "")
  } else if function == "[A]确认收到书籍" {
    return FsmEvent(stub, args, "[A]确认收到书籍")
  } else if function == "[A]确认收到书籍超时" {
    return FsmEvent(stub, args, "[A]确认收到书籍超时")
  } else if function == "[C]将书款返回给李华" {
    return FsmEvent(stub, args, "[C]将书款返回给李华")
  } else if function == "" {
    return FsmEvent(stub, args, "")
...
func (this *Routers) [A]预存书款100元((stub shim.ChaincodeStubInterface,
args []string) (pb.Response, string) {
  return shim.Success(nil), "Done"
}


func (this *Routers) [A]预存书款100元超时((stub shim.ChaincodeStubInterface,
args []string) (pb.Response, string) {
  return shim.Success(nil), "Done"
}
...
func (this *Routers) [A]确认收到书籍((stub shim.ChaincodeStubInterface,
args []string) (pb.Response, string) {
  return shim.Success(nil), "Done"
}


func (this *Routers) [A]确认收到书籍超时((stub shim.ChaincodeStubInterface,
args []string) (pb.Response, string) {
  return shim.Success(nil), "Done"
}


func (this *Routers) [C]将书款返回给李华((stub shim.ChaincodeStubInterface,
args []string) (pb.Response, string) {
  return shim.Success(nil), "Done"
}
